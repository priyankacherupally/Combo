...
// ── Shell: white topbar + scrollable main (no side nav) ──────
function StShell({ active = 'training', breadcrumb, onHome, onLogout, onNav, maxWidth = 1180, lang, setLang, children }) {
  return (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {lang && setLang && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Icon name="globe" size={18} style={{ color: 'var(--ink-faint)' }} />
              <LangPills lang={lang} onChange={setLang} compact />
            </div>
          )}
          <span style={{ width: 1, height: 26, background: 'var(--line)' }} />
          <button className="st-icon-btn" title="Log out" onClick={onLogout}><Icon name="logout" size={22} /></button>
          <button className="st-icon-btn" title="Home" onClick={onHome}><Icon name="home" size={22} /></button>
        </div>
      </header>
      <main className="scroll" style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ padding: '22px 30px 48px', maxWidth, margin: '0 auto' }}>
          {breadcrumb && (
            <div style={{ fontSize: '0.88rem', color: 'var(--ink-faint)', fontWeight: 500, marginBottom: 16 }}>
              {breadcrumb.map((b, i) => (
                <span key={i}>
                  {i > 0 && <span style={{ margin: '0 8px' }}>›</span>}
                  <span style={{ textDecoration: i === 0 ? 'underline' : 'none', color: i === breadcrumb.length - 1 ? 'var(--ink-soft)' : 'var(--ink-faint)' }}>{b}</span>
                </span>
              ))}
            </div>
          )}
