...
function Admin({ lang, setLang, notify, onExitToApp, onPickWorker }) {
  ...
  return (
    <StShell onHome={() => setView('library')} onLogout={onExitToApp} breadcrumb={crumb} maxWidth={view === 'editor' ? 1640 : 1180} lang={lang} setLang={setLang}>
...
function VideoLibrary({ lang, videos, onUpload, onOpen, onDash, onReportGo, onPickWorker }) {
  ...
  return (
    <div style={{ animation: 'suno-fade-up .4s both' }}>
      <StPageHeader title="Training Videos"
        right={<div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <StButton kind="outline" icon="chart" onClick={onDash} style={{ fontSize: '1.02rem', padding: '12px 22px' }}>Dashboard</StButton>
          <StButton kind="outline" icon="doc" onClick={onReportGo} style={{ fontSize: '1.02rem', padding: '12px 22px' }}>Completion report</StButton>
        </div>} />
      ...
      <button onClick={pickFile} style={{ width: '100%', border: '2px dashed var(--line)', borderRadius: 'var(--r-lg)',
        background: 'var(--surface)', cursor: 'pointer', padding: '46px 20px', marginBottom: 26,
        ...
        {videos.map((v) => {
          ...
          return (
            <button key={v.id} onClick={() => onOpen(v)} style={{ textAlign: 'left', cursor: 'pointer', borderRadius: 'var(--r-lg)',
              width: 264, flexShrink: 0, scrollSnapAlign: 'start',
...
function VideoEditor({ video, lang, setLang, notify, onBack, onSave }) {
  ...
  return (
    <div style={{ animation: 'suno-fade-up .4s both' }}>
      <StPageHeader title={title || 'Training video'} onBack={() => { stopClock(); stopSpeaking(); onBack(); }}
        right={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 13px', borderRadius: 999, background: 'var(--accent-wash)', color: 'var(--accent)', fontWeight: 700, fontSize: '0.84rem' }}>
          <Icon name="mic" size={16} /> {VOICE_LABEL}
        </span>} />
...
function TrainingDash({ lang, onBack, onNew, onReport }) {
  ...
  return (
    <div style={{ animation: 'suno-fade-up .4s both' }}>
      <StPageHeader title="Training Overview" onBack={onBack}
        right={<div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <StButton icon="upload" onClick={onNew}>Upload Video</StButton>
          <StDateRange />
        </div>} />
            ...
            {[['te', 20], ['hi', 4], ['en', 5]].map(([lg, n]) => (
              <div key={lg} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  ...
                  <span className={langClass(lg)} style={{ fontWeight: 600 }}>{LANG_NAME[lg]}</span>
                </span>
...
function CompletionReport({ lang, onBack }) {
  return (
    <div style={{ animation: 'suno-fade-up .4s both' }}>
      <StPageHeader title="Employee Completion" onBack={onBack}
        right={<div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <StDateRange />
          <button className="st-icon-btn" title="Export PDF"><Icon name="pdf" size={22} /></button>
          <button className="st-icon-btn" title="Export CSV"><Icon name="download" size={22} /></button>
        </div>} />
      <StCard pad={0}>
        <div style={{ overflowX: 'auto' }}>
          <table className="st-table">
