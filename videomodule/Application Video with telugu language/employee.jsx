  ...
  return (
    <div className="scroll" style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: 920, margin: '0 auto', padding: '20px 20px 48px' }}>
        {/* top bar */}
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, marginBottom: 22, flexWrap: 'wrap' }}>
          <button onClick={onLogout} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}>
            <Avatar name={employee.name} initials={employee.initials} hue={employee.hue} size={50} />
            <div>
              <div style={{ fontSize: '0.82rem', color: 'var(--ink-faint)', fontWeight: 600 }} className={langClass(lang)}>{t('hello')}</div>
              <div style={{ fontWeight: 800, fontSize: '1.12rem', lineHeight: 1.1 }}>{employee.name.split(' ')[0]}</div>
            </div>
          </button>
...
function CategoryScreen({ catId, lang, t, completed, offline, isAccessible, onBack, onOpenLesson, onToggleOffline, notify }) {
  ...
  return (
    <div className="scroll" style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '18px 20px 44px' }}>
